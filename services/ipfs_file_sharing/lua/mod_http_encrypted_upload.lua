  ...
  local secret = generate();
	...
	local reply = st.reply(stanza)
		:tag("slot", { xmlns = namespace })
			:tag("get", { url = slot_url }):text_tag('header', secret, { name = "secret" }):up():up()
			:tag("put", { url = slot_url }):text_tag('header', secret, { name = "secret" }):up():up()
		:up();
	origin.send(reply);
	return true;
end);
  ...
  local secret = generate();
	...
	local reply = st.reply(stanza)
		:tag("slot", { xmlns = legacy_namespace })
			:tag("get"):text_tag('header', secret, { name = "secret" }):up():text(slot_url):up()
			:tag("put"):text_tag('header', secret, { name = "secret" }):up():text(slot_url):up()
		:up();
	origin.send(reply);
	return true;
end);
